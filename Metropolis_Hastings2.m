...
[f(1), mask_row, mask_col, mask_square] = evalFunc(grids(:, :, 1));

i = 2;
while(i <= max_chain_length && f(i-1) ~= 0)
	y = test_proposition_good(grids(:, :, i-1), row_not_def, col_not_def, mask_row, mask_col, mask_square, initial_grid);
	[f_y, mask_row, mask_col, mask_square] = evalFunc(y);
