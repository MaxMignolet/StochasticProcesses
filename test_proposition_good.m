function s = test_proposition_good(grid, row_not_def, col_not_def, mask_row, mask_col, mask_square, grid_init)
...
while grid(r1, c1) == grid(r2, c2)
    ...
    if rand() > proba_take_conflic
        index2 = randi(length(row_to_swap));
        r2 = row_to_swap(index2);
        c2 = col_to_swap(index2);
    else
        index2 = randi(length(row_not_def));
        r2 = row_not_def(index2);
        c2 = col_not_def(index2);
    end
end
